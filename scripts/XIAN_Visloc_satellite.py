...
def crop_tif_to_tiles(tif_path, out_dir, id, coord_range, tile_size=512, stride=256, start_tile_id=1, record_txt_path=None, target_size=None):
    ...
    with rasterio.open(tif_path) as src:
        ...
        print('\n The tile num is: {}'.format(total_tiles))
...
if __name__ == "__main__":
    ...
    # Setting
    name = 'Xian'  # Xian | Weinan

    data_root = "data/XIAN_Visloc/" + name
    ...
    if name == 'Xian':
        DATA_LIST = [18, 19]
        satellite_id = [0 for _ in range(17)] + [3,4]
        space_resolution = [0 for _ in range(17)] + [0.126,0.134]

    elif name == 'Weinan':
        DATA_LIST = [1, 2]
        satellite_id = [1, 2]
        space_resolution = [0.126, 0.134]
    ...
    for i in range(1,20):
        ...
        if stride_set is None:
            ...
            Threshold = 0.04*(a*b)**(1/3) # c=0.004
        ...
        if Crop:
            ...
            crop_tif_to_tiles(image_path, out_path, id=i, coord_range=coord_range, tile_size=tile_size, stride=stride, record_txt_path=satellite_coordinates_path, target_size=target_size)
